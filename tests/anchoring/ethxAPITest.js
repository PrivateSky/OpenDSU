...
assert.callback("ETHx API test", (callback) => {
    dc.createTestFolder('ethx', async (err, folder) => {
        ...
        assert.equal(lastVersion, signedHashLinkSSI1.getIdentifier(true));
        let versions = await $$.promisify(anchoringX.getAllVersions)(anchorSSI.getIdentifier());
        assert.equal(versions.length, 1);
        assert.equal(versions[0], signedHashLinkSSI1.getIdentifier());
        ...
        assert.equal(versions[1], signedHashLinkSSI1.getIdentifier());
